...
doc_splitter = re.compile(r"^(?:\d+)\.", re.MULTILINE) #tach theo Khoan
doc_splitter_roman = re.compile(r"^(?:[IVX]+)\.", re.MULTILINE) #tach theo Khoan
#Chi lay ten Dieu
...
starts_roman = [match.span()[0] for match in doc_splitter_roman.finditer(text)] + [len(text)]
sections_roman = [text[starts_roman[idx]:starts_roman[idx+1]] for idx in range(len(starts_roman)-1)]
for section_roman in sections_roman:
    section_roman_head = section_roman.splitlines()[0]  #chi lay so dieu
    print([section_roman_head])
